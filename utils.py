...
def stackImages(imgArray,scale,lables = []):
    """Stack multiple images into one display"""
    rows = len(imgArray)
    cols = len(imgArray[0])
...
def rectContour(contours):
    ...
    for i in contours:
        ...
        if area > 50:  # Increased minimum area
            peri = cv2.arcLength(i, True)
            approx = cv2.approxPolyDP(i, 0.02 * peri, True)
            
            # Check for both rectangles and circles
            if len(approx) == 4:  # Rectangle
                rectCon.append(i)
            elif len(approx) > 4:  # Circle
                # Calculate circularity
                circularity = 4 * np.pi * area / (peri * peri)
                if circularity > 0.8:  # More circular shapes
                    rectCon.append(i)
...
def drawGrid(img,questions=5,choices=5):
    """Draw grid lines on the answer sheet"""
...
def showAnswers(img,myIndex,grading,ans,questions=5,choices=5):
    """Display answers and grading on the image"""
    secW = int(img.shape[1]/questions)
    secH = int(img.shape[0]/choices)
